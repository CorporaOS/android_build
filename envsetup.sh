...
function build_build_var_cache()
{
    ...
    cached_vars=(`cat $T/build/envsetup.sh $T/vendor/corpora/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/_get_build_var_cached/) print $(i+1)}' | sort -u | tr '\n' ' '`)
    cached_abs_vars=(`cat $T/build/envsetup.sh $T/vendor/corpora/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/_get_abs_build_var_cached/) print $(i+1)}' | sort -u | tr '\n' ' '`)
...
{
    if (echo -n $1 | grep -q -e "^corpora_") ; then
        CORPORA_BUILD=$(echo -n $1 | sed -e 's/^corpora_//g')
    else
        CORPORA_BUILD=
    fi
    export CORPORA_BUILD
    ...
    if [[ -z "${ANDROID_QUIET_BUILD}" && -z "${CORPORA_BUILD}" ]]; then
...
. $ANDROID_BUILD_TOP/vendor/corpora/build/envsetup.sh
